...
const items: MenuProps['items'] = [
  {
    label: <Space> 刷新页面 </Space>,
    key: '0',
  },
  {
    label: <Space> 关闭当前 </Space>,
    key: '1',
  },
  {
    label: <Space> 关闭其他 </Space>,
    key: '2',
  },
  {
    label: <Space> 全部关闭 </Space>,
    key: '3',
  },
];
...
const MainContent: React.FC<PropsInterface> = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      matchingMenuItem &&
      tabList.findIndex((J) => J.key === location.pathname) === -1 &&
      location.pathname != '/' &&
      location.pathname != '/home' &&
      matchingMenuItem?.component != 'Layout'
    ) {
      setTabList([
        ...tabList,
        {
  ...
  const handleTabRemove = (key: string) => {
    ...
    const newTabList = [
      ...tabList.slice(0, tabIndex),
      ...tabList.slice(tabIndex + 1),
    ];
        ...
        breadcrumbRender: (props) => {
          return (
            props?.breadcrumb?.items && (
              <Breadcrumb item={props.breadcrumb.items} />
            )
          );
        },
      }}
